    ...
    void calculate_antinodes(vector<vector<bool>> &antinode_map) {
        for (int p1 = 0; p1 < (int)this->coordinates.size() - 1; p1++){
            for (int p2 = p1 + 1; p2 < (int)this->coordinates.size(); p2++){
                ...
                pii antinode1 = this->coordinates[p1];
                while (in_bound(antinode1)){
                    antinode_map[antinode1.first][antinode1.second] = true;
                    antinode1 = get_new_point(antinode1, idiff);
                }

                pii antinode2 = this->coordinates[p2];
                while (in_bound(antinode2)){
                    antinode_map[antinode2.first][antinode2.second] = true;
                    antinode2 = get_new_point(antinode2, diff);
